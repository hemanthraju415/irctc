 ...
 */
@WebServlet(asyncSupported = true, urlPatterns = { "/bookticket" }, loadOnStartup = 1)
public class TicketBooking extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			psgrst.executeBatch();
			out.println("<body>" + "<table style=" + "text-align:'center'><caption>ticket details</caption>" + "<thead>"
					+ "                <th>" + "                    train no:" + train_no + "                </th>"
					+ "                <th>" + "                    pnr no:" + pnr_no + "                </th>"
					+ "                <th>" + "                    ticketno:" + ticket_no + "                </th>"
					+ "                <th>" + "                    from:" + from + "                </th>"
					+ "                <th>" + "                </th>" + "                <th> to:" + to + "</th>"
					+ "            </thead>" + "            <tbody>" + "                <tr>"
					+ "                    <th>" + "                        name" + "                    </th>"
					...
					+ "                </tr>");
			for (int i = 0; i < name_arr.size(); i++) {
				out.println("<tr><td>" + name_arr.get(i) + "</td>" + "<td>" + age_arr.get(i) + "</td>" + "<td>"
						+ gender_arr.get(i) + "</td>" + "</tr>");
			}
